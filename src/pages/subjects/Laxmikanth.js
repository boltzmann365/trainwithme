...
const Laxmikanth = () => {
  ...
  const fetchSingleMCQ = async (chapter = null, questionIndex = 0, count = 1, retryCount = 0, forceGenerate = false) => {
    ...
    const query = chapterName === "entire-book"
      ? `Generate ${count} MCQ from the entire Laxmikanth's Indian Polity book, covering constitutional and political topics comprehensively. Ensure the MCQ spans diverse topics across the book’s content, supplemented by internet resources and general knowledge, avoiding repetition of topics from the previous ${index} questions.`
      : `Generate ${count} MCQ from ${chapterName} of Laxmikanth's Indian Polity book. Use the chapter content as the primary source, supplemented by internet resources and general knowledge to ensure uniqueness and depth.`;
        ...
        {!testStarted ? (
          <div className="h-[calc(100vh-4rem)] w-full overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 px-2 sm:px-4 lg:px-8">
              {Object.keys(chapterNumbers).map((chapter) => (
                <div
                  key={chapter}

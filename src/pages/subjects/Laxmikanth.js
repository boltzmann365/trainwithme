...
const Laxmikanth = () => {
  ...
  // FIX #1: Added a cleanup function to the initial useEffect.
  // This makes the component compatible with React's StrictMode in development.
  useEffect(() => {
  ...
  const lastAttemptedIndex = userAnswers
    .slice()
    .reverse()
  ...
  const lastActiveIndex =
    lastAttemptedIndex === -1
      ? -1
      : userAnswers.length - 1 - lastAttemptedIndex;
        ...
        >
          ...
          >
            ...
            <div className="flex justify-center gap-4 w-full max-w-sm">
              <button
                onClick={() => {
                  // FIX #2: Reset the ref before starting a new test from the popover
                  isTestStarted.current = false; 
                  startTest(
                    pendingMode,

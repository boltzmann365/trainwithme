...
const DishaCSAT = () => {
  ...
  useEffect(() => {
    console.log("DishaCSAT component mounted, showModePopup:", showModePopup);
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      ...
      body: JSON.stringify({ userId, book: "CSAT", requestedCount: validRequestedCount }),
    });
 ...
 const fetchUserMCQsForLawMode = async (batchStart = 0) => {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId, book: "CSAT", requestedCount: LAW_MODE_BATCH_SIZE }),
    });

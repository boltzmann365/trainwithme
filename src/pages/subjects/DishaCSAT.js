...
const DishaCSAT = () => {
  ...
  const fetchSingleMCQ = async (chapter = null, questionIndex = 0, count = 1, retryCount = 0, forceGenerate = false) => {
    ...
    const query = chapterName === "entire-book"
      ? `Generate ${count} MCQ from the entire Disha IAS Previous Year Papers (CSAT Section), covering CSAT topics comprehensively. Ensure the MCQ spans diverse topics across the book’s content, supplemented by internet resources and general knowledge, avoiding repetition of topics from the previous ${index} questions.`
      : `Generate ${count} MCQ from ${chapterName} of the Disha IAS Previous Year Papers (CSAT Section). Use the chapter content as the primary source, supplemented by internet resources and general knowledge to ensure uniqueness and depth.`;

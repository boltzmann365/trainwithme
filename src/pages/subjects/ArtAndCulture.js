...
const ArtAndCulture = () => {
  ...
  useEffect(() => {
    console.log("ArtAndCulture component mounted, showModePopup:", showModePopup);
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      ...
      body: JSON.stringify({ userId, book: "ArtAndCulture", requestedCount: validRequestedCount }),
    });
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId, book: "ArtAndCulture", requestedCount: LAW_MODE_BATCH_SIZE }),
    });

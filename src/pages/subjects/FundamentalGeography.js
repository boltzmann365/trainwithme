...
const FundamentalGeography = () => {
  ...
  const fetchSingleMCQ = async (chapter = null, questionIndex = 0, count = 1, retryCount = 0, forceGenerate = false) => {
    ...
    const query = chapterName === "entire-book"
      ? `Generate ${count} MCQ from the entire NCERT Class 11th Fundamentals of Physical Geography book, covering topics comprehensively. Ensure the MCQ spans diverse topics across the book’s content, supplemented by internet resources and general knowledge, avoiding repetition of topics from the previous ${index} questions.`
      : `Generate ${count} MCQ from ${chapterName} of the NCERT Class 11th Fundamentals of Physical Geography book. Use the chapter content as the primary source, supplemented by internet resources and general knowledge to ensure uniqueness and depth.`;
  ...
  const startChapterMCQTest = async (chapter) => {
    ...
    if (initialMCQs && !initialMCQs.error && initialMCQs.length === 2) {
      ...
      setNextButtonState("white"); // Ensure button starts white
      setIsFetchingNext(false); // Ensure fetching state is false
      console.log(`startChapterMCQTest: Displayed MCQ Expeditions for ${chapter}`);
      console.log(`Initial state - questions: ${questions.length}, bufferedMCQs: ${bufferedMCQs.length}, nextButtonState: ${nextButtonState}, isFetchingNext: ${isFetchingNext}`);
    } else {
...
export default FundamentalGeography;
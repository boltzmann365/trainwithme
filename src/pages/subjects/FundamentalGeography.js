...
const FundamentalGeography = () => {
  ...
  useEffect(() => {
    console.log("FundamentalGeography component mounted, showModePopup:", showModePopup);
  ...
  const fetchUserMCQs = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
        ...
        body: JSON.stringify({ userId, book: "FundamentalGeography", requestedCount: validRequestedCount }),
      });
  ...
  const fetchUserMCQsForLawMode = async (batchStart = 0) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId, book: "FundamentalGeography", requestedCount: LAW_MODE_BATCH_SIZE }),
      });

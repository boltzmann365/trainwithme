...
const VisionIasDec2024 = () => {
  ...
  useEffect(() => {
    console.log("VisionIasDec2024 component mounted, showModePopup:", showModePopup);
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      ...
      body: JSON.stringify({ userId, book: "CurrentAffairs", requestedCount: validRequestedCount }),
    });
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId, book: "CurrentAffairs", requestedCount: LAW_MODE_BATCH_SIZE }),
    });

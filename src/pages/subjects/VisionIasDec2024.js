...
const VisionIasDec2024 = () => {
  ...
  const fetchSingleMCQ = async (chapter = null, questionIndex = 0, count = 1, retryCount = 0, forceGenerate = false) => {
    ...
    const query = chapterName === "entire-book"
      ? `Generate ${count} MCQ from the entire Vision IAS Current Affairs Magazine for December 2024, covering diverse topics comprehensively. Ensure the MCQ spans various themes (e.g., polity, economy, environment), supplemented by internet resources and general knowledge, avoiding repetition of topics from the previous ${index} questions and focusing on events up to December 2024.`
      : `Generate ${count} MCQ from ${chapterName} of the Vision IAS Current Affairs Magazine for December 2024. Use the magazine content as the primary source, supplemented by internet resources and general knowledge to ensure uniqueness, depth, and recency (events up to December 2024).`;
...
export default VisionIasDec2024;
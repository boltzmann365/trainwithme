...
const IndianGeography = () => {
  ...
  const fetchSingleMCQ = async (chapter = null, questionIndex = 0, count = 1, retryCount = 0, forceGenerate = false) => {
    ...
    const query = chapterName === "entire-book"
      ? `Generate ${count} MCQ from the entire Indian Geography Book, which covers the NCERT Class 11th Indian Geography comprehensively. Ensure the MCQ spans diverse topics across the book’s content, supplemented by internet resources and general knowledge, avoiding repetition of topics from the previous ${index} questions.`
      : `Generate ${count} MCQ from ${chapterName} of the Indian Geography Book, which covers the NCERT Class 11th Indian Geography. Use the chapter content as the primary source, supplemented by internet resources and general knowledge to ensure uniqueness and depth.`;

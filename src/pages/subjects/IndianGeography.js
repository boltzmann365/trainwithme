...
const IndianGeography = () => {
  ...
  useEffect(() => {
    console.log("IndianGeography component mounted, showModePopup:", showModePopup);
  ...
  const fetchUserMCQs = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
        ...
        body: JSON.stringify({ userId, book: "IndianGeography", requestedCount: validRequestedCount }),
      });
  ...
  const fetchUserMCQsForLawMode = async (batchStart = 0) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId, book: "IndianGeography", requestedCount: LAW_MODE_BATCH_SIZE }),
      });
 ...
 const handleGoBack = () => {
  if (testStarted) {
    console.log("Exiting test mode and returning to mode selection popup");
    resetTest(); // This will set testStarted to false and show the mode popup
  } else {
    console.log("Navigating back to previous page from mode selection popup");
    navigate(-1); // Only navigate back if not in a test (i.e., on the mode popup)
  }
};
  useEffect(() => {

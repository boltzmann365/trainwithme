...
const TamilnaduHistory = () => {
  ...
  // Define 19 chapters for Tamilnadu History as provided
  const chapterNumbers = {
    "Early India: From the Beginnings to the Indus Civilisation": "Chapter 1",
    "Early India: The Chalcolithic, Megalithic, Iron Age and Vedic Cultures": "Chapter 2",
    "Rise of Territorial Kingdoms and New Religious Sects": "Chapter 3",
    "Emergence of State and Empire": "Chapter 4",
    "Evolution of Society in South India": "Chapter 5",
    "Polity and Society in Post-Mauryan Period": "Chapter 6",
    "The Guptas": "Chapter 7",
    "Harsha and Rise of Regional Kingdoms": "Chapter 8",
    "Cultural Development in South India": "Chapter 9",
    "Advent of Arabs and Turks": "Chapter 10",
    "Later Cholas and Pandyas": "Chapter 11",
    "Bahmani and Vijayanagar Kingdoms": "Chapter 12",
    "Cultural Syncretism: Bhakti Movement in India": "Chapter 13",
    "The Mughal Empire": "Chapter 14",
    "The Marathas": "Chapter 15",
    "The Coming of the Europeans": "Chapter 16",
    "Effects of British Rule": "Chapter 17",
    "Early Resistance to British Rule": "Chapter 18",
    "Towards Modernity": "Chapter 19",
  };

  // Unit information for reference (not used in display for now)
  /*
  const chapterUnits = {
  ...
  */
  ...
  // Fetch cached MCQs from /user/mcqs
  const fetchSingleMCQ = async (count = 1, retryCount = 0) => {
    const maxRetries = 2;
    const chapterName = "entire-book";

    console.log(`fetchSingleMCQ: Requesting ${count} MCQ${count > 1 ? 's' : ''} for entire book`);

    try {
      const payload = {
        category: "TamilnaduHistory",
        chapter: chapterName,
        count
      };
      console.log(`fetchSingleMCQ: Sending payload:`, payload);
      const res = await fetch(`${API_URL}/user/mcqs`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        if (res.status === 404) {
          return { error: `No MCQs available for Tamilnadu History entire book` };
        }
        if (res.status === 500 && retryCount < maxRetries) {
          console.log(`Retrying fetchSingleMCQ, attempt ${retryCount + 1}/${maxRetries}`);
          return await fetchSingleMCQ(count, retryCount + 1);
        }
      ...
      console.error(
        `fetchSingleMCQ: Error fetching ${count} MCQ${count > 1 ? 's' : ''} for entire book:`,
        error.message
      );
  ...
  const startMCQTest = async () => {
    ...
    const initialMCQs = await fetchSingleMCQ(2);
    setLoading(false);
    isFetchingRef.current = false;
  ...
  const handleNextQuestion = async () => {
    ...
    if (bufferedMCQs.length > 0) {
      ...
      console.log(`handleNextQuestion: Fetching 1 new MCQ for index ${questions.length}`);
      const newMCQ = await fetchSingleMCQ(1);
      ...
      if (newMCQ && !newMCQ.error) {
        ...
        setError(newMCQ?.error || "Failed to fetch a new MCQ.");
        setNextButtonState("white"); // Reset to white to allow retry
        setIsFetchingNext(false);
      }
    } else {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 px-2 sm:px-4 lg:px-8">
              {Object.keys(chapterNumbers).map((chapter, index) => (
                <div
                  ...
                  onClick={startMCQTest} // Currently, all topics trigger the entire book test
                >

...
const TamilnaduHistory = () => {
  ...
  useEffect(() => {
    console.log("TamilnaduHistory component mounted, showModePopup:", showModePopup);
 ...
 const fetchUserMCQs = async () => {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      ...
      body: JSON.stringify({ userId, book: "TamilnaduHistory", requestedCount: validRequestedCount }),
    });
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId, book: "TamilnaduHistory", requestedCount: LAW_MODE_BATCH_SIZE }),
    });

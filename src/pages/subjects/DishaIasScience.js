...
const DishaIasScience = () => {
  ...
  const [expandedParts, setExpandedParts] = useState({
    part1: true, // First unit expanded by default
  });
  ...
  const fetchSingleMCQ = async (chapter = null, questionIndex = 0, count = 1, retryCount = 0, forceGenerate = false) => {
    ...
    const query = chapterName === "entire-book"
      ? `Generate ${count} MCQ from the entire Science Book, which covers Physics, Chemistry, Biology, and Science & Technology from the Disha IAS Previous Year Papers. Ensure the MCQ spans diverse topics across the book’s content, supplemented by internet resources and general knowledge, avoiding repetition of topics from the previous ${index} questions.`
      : `Generate ${count} MCQ from ${chapterName} of the Science Book, which covers Physics, Chemistry, Biology, and Science & Technology from the Disha IAS Previous Year Papers. Use the chapter content as the primary source, supplemented by internet resources and general knowledge to ensure uniqueness and depth.`;

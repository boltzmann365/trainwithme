...
const Spectrum = () => {
  ...
  useEffect(() => {
    console.log("Spectrum component mounted, showModePopup:", showModePopup);
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      ...
      body: JSON.stringify({ userId, book: "Spectrum", requestedCount: validRequestedCount }),
    });
 ...
 const fetchUserMCQsForLawMode = async (batchStart = 0) => {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/user/get-book-mcqs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId, book: "Spectrum", requestedCount: LAW_MODE_BATCH_SIZE }),
    });
